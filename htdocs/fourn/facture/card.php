	...
	if ($id > 0 || !empty($ref)) {
		...
		if ($action != 'presend') {
			...
			if (empty($reshook)) {
				...
				if ($object->status == FactureFournisseur::STATUS_VALIDATED && ($object->type == FactureFournisseur::TYPE_CREDIT_NOTE || $object->type == FactureFournisseur::TYPE_DEPOSIT || $object->type == FactureFournisseur::TYPE_STANDARD)) {
